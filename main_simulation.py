...
class MultithreadingManager:
    def __init__(self):
        ...
        self.digisim_pool = ThreadPoolExecutor(max_workers=1) # Needs to be processed one after the other
        self.ecsim_pool = ProcessPoolExecutor(max_workers=2)
        self.electrokitty_pool = ProcessPoolExecutor(max_workers=4)
        self.comsol_pool = ProcessPoolExecutor(max_workers=8)
        
        # Queues for each simulator
        self.digisim_queue = Queue()
        self.ecsim_queue = Queue()
        self.comsol_queue = Queue()
        self.electrokitty_queue = Queue()
    ...
    def start_all_queue_processors(self):
        ...
        self.queue_processors = [
            threading.Thread(target=self._process_digisim_queue, daemon=True),
            threading.Thread(target=self._process_ecsim_queue, daemon=True),
            threading.Thread(target=self._process_comsol_queue, daemon=True),
            threading.Thread(target=self._process_electrokitty_queue, daemon=True)
        ]
    ...
    def _process_digisim_queue(self):
        """Process digisim queue - single task at a time"""
        while not self.stop_queues:
            try:
                task_data = self.digisim_queue.get(timeout=1)
                
                self._update_active_tasks('digisim', 1)
                
                # Digisim runs single tasks
                future = self.digisim_pool.submit(
                    task_data['func'], 
                    *task_data['args'], 
                    **task_data['kwargs']
                )
                
                # Add callbacks for completion/error
                future.add_done_callback(
                    lambda f, td=task_data: self._handle_task_completion(f, td, 'digisim')
                )
                
            except Empty:
                continue
            except Exception as e:
                print(f"Error in digisim queue processor: {e}")
    
    def _process_ecsim_queue(self):
        """Process ecsim queue - multiple parallel tasks"""
        while not self.stop_queues:
            try:
                task_data = self.ecsim_queue.get(timeout=1)
                
                self._update_active_tasks('ecsim', 1)
                
                # ECSIM can run parallel tasks
                future = self.ecsim_pool.submit(
                    task_data['func'], 
                    *task_data['args'], 
                    **task_data['kwargs']
                )
                ...
                # Add callbacks for completion/error
                future.add_done_callback(
                    lambda f, td=task_data: self._handle_task_completion(f, td, 'ecsim')
                )
                
            except Empty:
                continue
            except Exception as e:
                print(f"Error in ecsim queue processor: {e}")
    
    def _process_electrokitty_queue(self):
        """Process electrokitty queue - multiple parallel tasks"""
        while not self.stop_queues:
            try:
                task_data = self.electrokitty_queue.get(timeout=1)
                
                self._update_active_tasks('electrokitty', 1)
                
                # Electrokitty can run parallel tasks
                future = self.electrokitty_pool.submit(
                    task_data['func'], 
                    *task_data['args'], 
                    **task_data['kwargs']
                )
                ...
                # Add callbacks for completion/error
                future.add_done_callback(
                    lambda f, td=task_data: self._handle_task_completion(f, td, 'electrokitty')
                )
                
            except Empty:
                continue
            except Exception as e:
                print(f"Error in electrokitty queue processor: {e}")
...
class GeneralSimulationRunner:
    ...
    def generate_all_param_maps(self):
        """Generate all parameter maps and save them"""
        print(f"Generating {self.num_simulations} parameter maps for {self.mechanism} mechanism sequentially...")
        ...
        for model_id in range(self.num_simulations):
            ...
            print(f"Generated parameters for model {model_id}")
    ...
    def run_all_simulations_parallel(self):
        ...
        for model_id in range(self.num_simulations):
            ...
            # Submit digisim task
            self.thread_manager.submit_task('digisim', {
                'model_id': model_id,
                'func': run_digisim_simulation,
                'args': (self.G_with_intermediates, param_map, comsol_params, self.scan_rates),
                'kwargs': {},
                'callback': self._save_simulator_results,
                'error_callback': self._handle_simulator_error
            })
            
            # Submit ecsim task (parallel capable)
            self.thread_manager.submit_task('ecsim', {
                'model_id': model_id,
                'func': run_ecsim_simulation,
                'args': (self.G_with_intermediates, param_map, comsol_params, self.scan_rates),
                'kwargs': {},
                'callback': self._save_simulator_results,
                'error_callback': self._handle_simulator_error
            ...
            # Submit electrokitty task (parallel capable)
            self.thread_manager.submit_task('electrokitty', {
                'model_id': model_id,
    ...
    def _wait_for_queues_to_empty(self):
        ...
        while True:
            ...
            # Get queue sizes
            q_sizes = {
                'digisim': self.thread_manager.digisim_queue.qsize(),
                'ecsim': self.thread_manager.ecsim_queue.qsize(),
                'comsol': self.thread_manager.comsol_queue.qsize(),
                'electrokitty': self.thread_manager.electrokitty_queue.qsize()
            }
            
            print(f"Queues: Digisim={q_sizes['digisim']}(A:{active_tasks['digisim']}), "
                  f"Ecsim={q_sizes['ecsim']}(A:{active_tasks['ecsim']}), "
                  f"COMSOL={q_sizes['comsol']}(A:{active_tasks['comsol']}), "
                  f"Electrokitty={q_sizes['electrokitty']}(A:{active_tasks['electrokitty']})")
...
def rerun_failed_digisim(base_dir=".", runner=None):
    ...
    for model_dir in model_dirs:
        ...
        print(existing_scan_rates)
        print(existing_digisim_files)
